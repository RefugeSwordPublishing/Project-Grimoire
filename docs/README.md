...
- material-recycling-REQUEST.md, OPEN design ask (for Chat): a rewarding sink for surplus/throwaway crafted + gathered items in an idle crafter, WITHOUT just fauceting SM/GM (the Royal Merchant currency). Grounds it: there is no NPC vendor (Sell = an Exchange listing needing a buyer; Discard = delete for nothing), items carry baseSellValue + quality/tier + isProtected. Asks for what recycling outputs (scrap/materials, a bespoke token, talent XP, or a capped SM trickle), bulk + auto-recycle controls, a value floor below the Exchange, and economy-safety.
