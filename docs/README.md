...
- combat-attack-speed-REQUEST.md, design ask (requesting Chat). Weapon-type attack speed (Dagger>Sword, Wand>Staff with damage tradeoffs) + enemy attack-speed variance (attackCadence field already exists, needs authored values) + a tick-based timing model that auto-eat/DoTs/regen quantize to. As-built: player is a flat 2.0s interval for every weapon; damage is quality x tier (weaponType only sets favoured stat + accuracy). Also records the auto-eat rework (paid-only, 30/40/70 thresholds, potion-gated).
