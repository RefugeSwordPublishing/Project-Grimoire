...
- tier-quality-stat-curve.md, CANONICAL v1.0 (Chat, IMPLEMENTED): fixes bug #54. Tier now also raises a
  ...
  armour tier bonuses.
- hp-progression-REQUEST.md, OPEN design ask (for Chat): design an HP/survivability progression so players do not get squishier as enemy damage scales by tier, with tankier Grimoires getting more. Grounds it in the as-built HP model (VIT-only GetMaxHP; only Vanguard grants VIT milestones so Warden/Arcanist gain no permanent HP; no per-level stat growth; mitigation = rawDamage - defense*0.4; KO revives at 10%, no out-of-combat regen; Lifebinder 1.6x pool).
