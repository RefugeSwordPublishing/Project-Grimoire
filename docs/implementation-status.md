...
- **Combat loop — now SPEC'D (not yet built):** `combat-engagement-spec.md` v0.2 defines the concrete `EnterZone` loop (weighted-random spawn over `spawnWeight` → resolve idle/active → loot → repeat), elite cadence (6% + 0.1%/level, cap +9%; + Slaying-talent bonus), boss cadence (flat 1/20, active-only, 10-min despawn). Implement as `pg-combat-loop`. `EnterZone` still a stub.
