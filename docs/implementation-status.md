...
- **Combat loop — BUILT (idle MVP, in `CombatManager`):** `EnterZone` starts a semi-idle loop (runs even with the hub closed): `Update` ticks every 2s → `SpawnNext` (weighted-random over `spawnWeight`; elite roll = 6% + 0.1%/level cap +9%) → one attack exchange via `ResolveAttack`/`ResolveEnemyAttack` → on kill: roll `dropTable` into inventory, add Silver Marks, `CombatXP.AwardKill` to the equipped Grimoire, heal, next enemy. `OnCombatLog` event feeds the hub's status line while open. **Not yet:** active-play inputs / attunement bonuses, zone-boss spawns (active-only), Slaying-talent elite bonus, and a real over-the-shoulder combat view (loop is currently headless + log). Needs authored `EnemyData` per zone (`pg-p2-zones`) to fight real enemies.
