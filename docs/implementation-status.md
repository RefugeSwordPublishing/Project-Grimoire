...
- **Combat hub BUILT (`CombatHubUI.cs`):** nav → Combat lists Zones and Dungeons (sorted by tier) with lock state + required level via `ZoneAccess`; tapping an unlocked row calls `CombatManager.EnterZone`. Raids are a Phase-4 placeholder note. `BuildCombatTabUI` auto-populates the zone list from every `ZoneData` asset (currently Grimwood Fringe, Saltmarsh Shore). Replaces the old "Coming Soon" nav stub. **Next:** the actual in-zone combat loop / encounter flow after entering.
